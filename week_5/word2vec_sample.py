...
rc('font', family= font_manager
   .FontProperties(fname='C:/Windows/Fonts/NanumGothic.TTF')
   .get_name())

# tensorflow 연산에서 matplot 사용할 때 한글 처리

# 단어 벡터를 분석해 볼 임의의 문잘들
sentences = ["나 고양이 좋다",
            "나 강아지 좋다",
            "나 동물 좋다",
            "강아지 고양이 동물",
...
word_dict = {W: i for i, W in enumerate(word_list)} # dict 타입으로 변경, 텐서플로는 dict로 소통
...
# 총 단어의 갯수와 임배딩 갯수를 크기로 하는 두개의 차원을 갖습니다.
# embedding vector 의 차원에서 학습할 입력값에 대한 행들을 뽑아옵니다.
"""
 예) embeddings     inputs      selected      # input[2, 3] 2행, 3행 출력
    [[1, 2, 3]  ->  [2, 3]  ->  [[2, 3, 4]
     [2, 3, 4]                   [3, 4, 5]]
     [3, 4, 5]
     [4, 5, 6]]     
