...
for dataset in combine:
     dataset['Title'] = dataset['Title'].replace(['Capt','Col','Don','Dr','Major','Rev','Jonkheer'],'Rare') # Rare 로 대체
     dataset['Title'] = dataset['Title'].replace(['Countess','Lady','Sir'],'Royal') # Royal 로 대체
     dataset['Title'] = dataset['Title'].replace('Mlle','Miss')
     dataset['Title'] = dataset['Title'].replace('Ms','Miss')
     dataset['Title'] = dataset['Title'].replace('Mme','Mrs')

train[['Title','Survived']].groupby(['Title'], as_index = False).mean()
# groupby 는 같은 값을 가지는 인스턴트를 묶어서 연산하라(mean 이면 평균구하기)
"""
    Title  Survived
0  Master  0.575000
1    Miss  0.702703
...
train = train.drop(['Name','PassengerId'], axis= 1) # 한꺼번에 두 개 삭제

# SVC() 에서 사용할 데스트 아이디 작성.... 검증 이후에 제출할 테스트값으로 사용함
...
# AgeGroup를 만듦. cut() 함수는 각 구간의 값을 특정값으로, -1부터 0: Unknow, 0부터 1: Baby so on
# 정의해주는 함수
import numpy as np
...
bar_chart('AgeGroup')
# unknown이 너무 많아 title을 보고 임의로 매핑
...
# 데이터 모델링
# **************************
train_data = train.drop('Survived', axis=1) # 0, 1은 이미 사망, 생존 값을 나타낸 답이므로 일단 삭제
target = train['Survived'] # 답안지이므로 Survived는 분리
...
clf = DecisionTreeClassifier()  # tree means, 하나만 가지고 판단
scoring = 'accuracy'
score = cross_val_score(clf, train_data, target, cv=k_fold,
                        n_jobs= 1, scoring= scoring)
print(round(np.mean(score) * 100, 2))

print('>>>랜덤포레스트 방식 검증')  # 81.15  # forest means, 여러개 가지고 판단
