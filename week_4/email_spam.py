...
def latters_only(astr):
    return astr.isalpha()
import nltk
# nltk.download('all') # 영어사전 다운로드, 한번 실행하고 나면 주석 처리
...
def get_posterior(term_document_matix, prior, likelihood):
    num_docs = term_document_matix.shape[0]
    posteriors = []
    for i in range(num_docs):
        posterior = {key: np.log(prior_label) \
                     for key, prior_label in prior.items()}
        for label, likelihood_label in likelihood.items():
            term_document_vector = term_document_matix.getrow(i)
